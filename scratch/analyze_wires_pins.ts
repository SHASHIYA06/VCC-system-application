...
async function main() {
    ...
    const distinctPinWireNos = await prisma.$queryRaw<CountResult[]>`
    ...
    const distinctEndpointPins = await prisma.$queryRaw<CountResult[]>`
    ...
    const unmatchedPins = await prisma.$queryRaw<UnmatchedPin[]>`
    ...
    unmatchedPins.forEach((row) => {
        console.log(`   Connector ${row.connectorCode} Pin ${row.pinNo}: WireNo = "${row.wireNo}"`);
    });

    const totalUnmatchedPins = await prisma.$queryRaw<CountResult[]>`
