...
import { Search, BookOpen, Cpu, FileText, Lightbulb, Zap, Shield, DoorOpen, Wind, Radio, Battery, Settings, Train, Link2 } from 'lucide-react';
      ...
      {activeTab === 'systems' && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {VCC_SYSTEMS.map((system) => {
            ...
            return (
              <div key={system.systemCode} className="glass-card overflow-hidden hover:scale-[1.02] transition-transform">
                ...
                <div className="p-6">
                  <div className="text-xs text-slate-500 mb-2">Chapter {system.chapter}</div>
                  <p className="text-sm text-slate-300 mb-4">{system.description}</p>
                  
                  <div className="mb-3">
                    <div className="text-xs font-semibold text-slate-400 uppercase mb-2">Drawings</div>
                    <div className="flex flex-wrap gap-1">
                      {system.drawings.slice(0, 4).map((dwg) => (
                        <span key={dwg} className="text-xs font-mono bg-slate-700/50 px-2 py-1 rounded text-cyan-400">
                          {dwg}
                        </span>
                      ))}
                      {system.drawings.length > 4 && (
                        <span className="text-xs text-slate-500">+{system.drawings.length - 4} more</span>
                      )}
                    </div>
                  </div>
                  
                  <div>
                    <div className="text-xs font-semibold text-slate-400 uppercase mb-2">Key Components</div>
                    <div className="flex flex-wrap gap-1">
                      {system.keyComponents.slice(0, 5).map((comp) => (
                        <span key={comp} className="text-xs bg-slate-700/30 px-2 py-1 rounded text-slate-300">
                          {comp}
                        </span>
                      ))}
                    </div>
